...
//                            +elderlyData.getEkcal()+", "+elderlyData.getElatitude()+", "+elderlyData.getElongitude(),Toast.LENGTH_LONG).show();
    ...
    public void emergencyState(ElderlyData elderlyData){

        ApiUtils.getElderlyService(getApplicationContext()).emergencyState(elderlyData).enqueue(new retrofit2.Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    Log.d("Push_EMER_SUC", response.message()); // 성공 : OK(200)
                    Toast.makeText(getApplicationContext(),"Put Success:"+response.message(),Toast.LENGTH_SHORT).show();
                    Toast.makeText(getApplicationContext(),elderlyData.getEkey()+", "+elderlyData.getEstep()+", "+elderlyData.getEpulse()+", "
                            +elderlyData.getEkcal()+", "+elderlyData.getElatitude()+", "+elderlyData.getElongitude(),Toast.LENGTH_LONG).show();
                } else {
                    Log.d("Push_EMER_NOT_SUC", response.message()); // 실패
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) { // 연결 X
                t.getLocalizedMessage();
                Log.d("Push_EMER_Fail", call.toString());
    ...
    public void processIntent(Intent intent) {
        if (intent != null){
            ...
            name = bundle.getString(bundle.getString("name"));
            ekey = bundle.getInt("ekey");
            elderlyData.setEkey(ekey);
            regid = bundle.getString("regid");

            Toast.makeText(this,name+"님 환영합니다." + "eKey:"+ekey+"regId:"+regid,Toast.LENGTH_SHORT).show();
        }
    }
